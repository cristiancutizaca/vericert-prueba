...
const CreateStudentForm: React.FC<CreateStudentFormProps> = ({
  ...
  const onSubmit: SubmitHandler<StudentFormData> = async (data) => {
    try {
      ...
      if (
        !Num(data.code) ||
        (data.code.length !== 8 && data.code.length !== 9 && data.code.length !== 10)
      ) {
        setError("code", {
          type: "manual",
          message:
            "El codigo debe contener solo números y exactamente 8 , 9 y 19 digitos",
        });
        return;
      }
      /*  if (!Num(data.hour)) {
      ...
      <div className="max-w-screen-lg mx-auto border p-4 rounded-xl">
        ...
        >
          <div className="flex justify-between col-span-2 whitespace-pre-wrap">
            <div className="mb-4">
              <label className="text-xs text-gray-600 dark:text-white font-bold">
                DNI:{" "}
              </label>
              <input
                {...register("documentNumber", { required: true })}
                className={`border text-gray-700 dark:text-gray-200 rounded-lg p-1 lg:w-36 w-24 ${
                  errors?.documentNumber ? "border-red-500" : ""
                }`}
              />
              {errors?.documentNumber && (
                <span className="text-xs font-mono block text-red-400">
                  {errors.documentNumber.message}
                </span>
              )}
            </div>
            <div className="mb-4">
              <label className="text-xs font-bold text-gray-600 dark:text-white">
                Código:{" "}
              </label>
              <input
                {...register("code", { required: true })}
                className={`border text-gray-700 dark:text-gray-200 rounded-lg p-1 lg:w-32 w-28 ${
                  errors?.code ? "border-red-500" : ""
                }`}
              />
              {errors?.code && (
                <span className="text-xs lg:ml-4 font-mono block text-red-400">
                  {errors.code.message}
                </span>
              )}
            </div>
          ...
          <div className="mb-4 text-xs col-span-full md:col-span-2 lg:col-span-3">
            <label className="block font-bold text-gray-600 dark:text-white">
              Nombres:{" "}
            </label>
            <input
              {...register("name", { required: true })}
              className="border text-gray-700 dark:text-gray-200 rounded-lg p-2 w-full"
            />
          </div>
          <div className="mb-4 col-span-full text-xs md:col-span-2 lg:col-span-3">
            ...
            <input
              {...register("activityAcademy", { required: true })}
              className="border text-gray-700 dark:text-gray-200 rounded-lg p-2 w-full"
            />
          </div>
          <div className="mb-4 col-span-full text-xs md:col-span-2 lg:col-span-3">
          ...
          <div className="flex justify-between col-span-2 whitespace-pre-wrap">
            <div className="mb-4">
              ...
              <input
                ...
                className={`border text-gray-700 dark:text-gray-200 rounded-lg p-1 lg:w-36 w-36 ${
            ...
            <div className="mb-4 lg:ml-3">
              ...
              <input
                ...
                className="border text-gray-700 dark:text-gray-200 rounded-lg p-1 lg:w-44 w-36"
              />
            </div>
          </div>
